 ...
 * (decoderawtransaction, sendrawtransaction)
 *
 * @param {Object} opts allows to skip certain tests. {@see VxxlTransaction#serialize}
 * @return {string}
 */
...
Transaction.prototype.fromObject = function fromObject(arg, opts) {
  ...
  _.each(transaction.inputs, function (input) {
    ...
    var txin;
    if (
        (script.isScriptHashOut() || script.isWitnessScriptHashOut()) &&
        input.publicKeys &&
        input.threshold
    ) {
      txin = new Input.MultiSigScriptHash(
        ...
        script.isWitnessPublicKeyHashOut() ||
...
/**
 * @typedef {Object} VxxlTransaction~fromObject
 ...
 * ```
 *
 * @param {(Array.<Transaction~fromObject>|VxxlTransaction~fromObject)} utxo
 * @param {Array=} pubkeys
 ...
 */
Transaction.prototype.associateInputs = function (
    utxos,
    pubkeys,
    threshold,
    opts
  ...
  for (let utxo of utxos) {
    const index = this.inputs.findIndex(
        (i) =>
            i.prevTxId.toString("hex") === utxo.txId &&
            i.outputIndex === utxo.outputIndex
    );
    indexes.push(index);
    if (index >= 0) {
  ...
  if (pubkeys && threshold) {
    if (utxo.script.isMultisigOut()) {
      clazz = MultiSigInput;
        ...
        utxo.script.isWitnessScriptHashOut()
    ) {
      clazz = MultiSigScriptHashInput;
    }
  } else if (
      utxo.script.isPublicKeyHashOut() ||
      utxo.script.isWitnessPublicKeyHashOut() ||
      utxo.script.isScriptHashOut()
  ) {
...
Transaction.prototype._getInputFrom = function (
    utxo,
    pubkeys,
    threshold,
    opts
...
/**
 * @typedef {Object} VxxlTransaction~toObject
...
Transaction.prototype.to = function (address, amount) {
  ...
  $.checkArgument(
      JSUtil.isNaturalNumber(amount instanceof BN ? amount.toNumber() : amount),
  ...
  if (changeAmount > 0) {
    ...
    this._addOutput(
        new Output({
          ...
          satoshis: new BN(changeAmount.toString(16), 16),
        })
    );
  } else {
    this._changeIndex = undefined;
...
Transaction.prototype.sign = function (privateKey, sigtype, signingMethod) {
  ...
  _.each(
      this.getSignatures(privateKey, sigtype, signingMethod),
      function (signature) {
...
Transaction.prototype.getSignatures = function (
    privKey,
    sigtype,
    signingMethod
) {
...
Transaction.prototype.isValidSignature = function (signature, signingMethod) {
  ...
  return this.inputs[signature.inputIndex].isValidSignature(
      self,
      signature,
      signingMethod
  );
